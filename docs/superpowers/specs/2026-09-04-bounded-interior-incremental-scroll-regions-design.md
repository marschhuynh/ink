...
**Date:** 2026-09-04  
**Status:** Proposed — user approved the design direction; written-spec review pending  
...
The correction must eliminate the observed one-frame flash of transcript content behind the composer in Terminus/Tabby over SSH and provide the same intermediate-state guarantee for ToolDetailModal borders, actions, footer rows, and surrounding application chrome.
...
Rows `[0, start)` and `[end, height)` are outside the terminal mutation. The latter includes the composer, modal footer/border, or other stable chrome.
...
5. Require the existing minimum matched-core threshold of at least half the frame and a contiguous qualifying span.
...
The model must support the subset used by this path:
...
- text and newline writes.

For every intermediate snapshot, cells outside the active detected band must remain byte-for-byte unchanged unless the ordinary diff explicitly rewrites a row because its desired content changed.

The model is a test helper, not production terminal emulation.
...
- composer border/text remains stationary when unchanged;
...
Render the real large-result ToolDetailModal through production incremental Ink mode. Scroll its VLBox/ScrollBox viewport and apply captured stdout bytes to the terminal-cell model.

Assert for every intermediate state:

- modal top and bottom borders remain fixed;
- title, actions/footer, and application rows outside the content viewport never receive content cells;
- scrollbar changes are repaired within the bounded region;
- final terminal cells match the intended frame;
...
- outer real VirtualizedList scrolling does not regress materially from a pre-change baseline;
- outer ToolDetailModal release benchmark remains within its existing gate.

Because host timing is noisy, byte-count and shifted-core non-retransmission are the primary deterministic performance assertions. Timing benchmarks are corroborating gates using the existing sample counts and thresholds.
